...
//  
//
//  Created by Simon Mcloughlin on 12/10/2022.
//
// 	Modified from: https://gist.github.com/oteronavarretericardo/7da7bdeadf3829b1cadf5a2a10e83d85
	...
	mutating func decode(_ type: [Any].Type) throws -> [Any] {
		...
		while isAtEnd == false {
			if let value = try? decode(Bool.self) {
				...
				//unavoidable, but should be fine. We return [Any]. An overload to return homegenous array would be nice.
				array.append(NSNull())
			} else {
				
				// Unsupported type, Decode a dummy object to force the array to move on to the next element
				let _ = try? decode(DummyCodable.self)
			}
		}
