...
public class DipDupClient {
	...
	*/
	...
	public func getLiquidityFor(address: String, completion: @escaping ((Result<GraphQLResponse<DipDupPosition>, ErrorResponse>) -> Void)) {
		let queryDict = ["query": "query { position(where: {traderId: {_eq: \"\(address)\"}, sharesQty: {_gt: \"0\"} }) { sharesQty, token { symbol, address, decimals}, exchange { name, address, tezPool, tokenPool, sharesTotal}}}"]
