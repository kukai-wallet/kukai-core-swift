...
public class TezosNodeClient {
	...
	*/
	public func estimate(operations: [Operation], withWallet wallet: Wallet, receivedSuggestedGas: Bool, completion: @escaping ((Result<[Operation], ErrorResponse>) -> Void)) {
		
		if let constants = self.networkConstants {
			self.estimate(operations: operations, constants: constants, withWallet: wallet, receivedSuggestedGas: receivedSuggestedGas, completion: completion)
	...
	private func estimate(operations: [Operation], constants: NetworkConstants, withWallet wallet: Wallet, receivedSuggestedGas: Bool, completion: @escaping ((Result<[Operation], ErrorResponse>) -> Void)) {
		getOperationMetadata(forWallet: wallet) { [weak self] (result) in
			switch result {
				case .success(let metadata):
					self?.feeEstimatorService.estimate(operations: operations, operationMetadata: metadata, constants: constants, withWallet: wallet, receivedSuggestedGas: receivedSuggestedGas, completion: completion)
