...
public class TzKTClient {
	...
	*/
		...
		*/
		...
		dispatchGroup.notify(queue: .main) { [weak self] in
			if let err = errorFound {
				...
				let account = Account(walletAddress: address, xtzBalance: tzktAccount.xtzBalance, xtzStakedBalance: tzktAccount.xtzStakedBalance, xtzUnstakedBalance: tzktAccount.xtzUnstakedBalance, xtzFinalisedBalance: .zero(), tokens: groupedData.tokens, nfts: groupedData.nftGroups, recentNFTs: groupedData.recentNFTs, liquidityTokens: [], delegate: tzktAccount.delegate, delegationLevel: tzktAccount.delegationLevel)
	...
	private func mergeTokenTransfersWithTransactions() {
		var transactionsToRemove: Set<Int> = []
		var transfersToRemove: Set<Int> = []
			...
			for (transactionIndex, transaction) in self.tempTransactions.enumerated() {
				...
				if transfer.transactionId == transaction.id {
					...
					if transaction.tokenTransfersCount == nil {
						...
						transfersToRemove.insert(transferIndex)
					} else {
						transactionsToRemove.insert(transactionIndex)
					}
				}
			}
		}
