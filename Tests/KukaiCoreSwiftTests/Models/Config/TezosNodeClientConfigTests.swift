...
class TezosNodeClientConfigTests: XCTestCase {
	...
	func testDefaults() {
		...
		XCTAssert(config1.primaryNodeURL.absoluteString == "https://\(config1.tezosChainName.rawValue).api.tez.ie", config1.primaryNodeURL.absoluteString)
		XCTAssert(config1.parseNodeURL?.absoluteString == "https://\(config1.tezosChainName.rawValue).api.tez.ie", config1.parseNodeURL?.absoluteString ?? "")
